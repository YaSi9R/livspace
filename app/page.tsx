...
export default function HomePage() {
  ...
  return (
    <div className="relative min-h-screen bg-[#fcfbfb] pb-20 lg:pb-0">
      ...
      {/* NAVIGATION HEADER */}
      <header
        className={`w-full transition-all duration-300 z-40 sticky top-0 bg-white/95 backdrop-blur-md border-b border-gray-100 ${
          scrolled
            ? "shadow-md py-3"
            : "py-4.5"
        }`}
          ...
          <div className="flex items-center gap-8">
            <a href="#" className="flex items-center gap-1">
              <span className="text-2xl font-black tracking-tight text-black flex items-center">
                LIVSPACE<span className="text-[#bf1e2e] font-extrabold">.</span>
              </span>
            </a>
                    ...
                    {cities.map((city) => (
                      <button
                        ...
                        className={`w-full text-left px-4 py-2 text-sm hover:bg-slate-50 transition-colors flex items-center justify-between ${
                          selectedCity === city ? "text-primary font-semibold bg-red-50/50" : "text-slate-700"
                        }`}
                      >
                        {city}
        ...
        {heroImages.map((image, index) => (
          <div
            ...
            className={`absolute inset-0 transition-opacity duration-1000 ease-in-out ${
              index === currentHeroIndex ? "opacity-40" : "opacity-0 pointer-events-none"
            }`}
          >
      ...
      <section id="budget-calculator" className="py-20 bg-slate-50 border-b border-gray-100 scroll-mt-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center max-w-2xl mx-auto mb-16">
            ...
            <p className="text-slate-600">
              Select elements, layout, and finish quality to view live estimations instantly. No sign-up required.
            </p>
          </div>
          ...
          <div className="grid lg:grid-cols-12 gap-8 items-stretch">
            ...
            {/* Calculator controls */}
            <div className="lg:col-span-7 bg-white p-8 rounded-2xl shadow-xl border border-slate-100 flex flex-col gap-6">
              
              {/* BHK Select */}
              <div>
                <h4 className="text-sm font-bold text-slate-900 uppercase tracking-wider mb-3 flex items-center gap-2">
                  <span className="w-5 h-5 rounded-full bg-red-50 text-primary text-xs flex items-center justify-center font-extrabold">1</span>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                  {["1 BHK", "2 BHK", "3 BHK", "4 BHK / Villa"].map((bhk) => (
                    <button
                      key={bhk}
                      onClick={() => setCalcBhk(bhk as any)}
                      className={`py-2 px-3 border rounded-xl text-xs font-bold transition-all ${
                        calcBhk === bhk
                          ? "border-primary bg-red-50/50 text-primary shadow-sm"
              ...
              <div>
                <h4 className="text-sm font-bold text-slate-900 uppercase tracking-wider mb-3 flex items-center gap-2">
                  <span className="w-5 h-5 rounded-full bg-red-50 text-primary text-xs flex items-center justify-center font-extrabold">2</span>
                  Choose Finish & Warranty Tier
                </h4>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  {[
                    { id: "Essential", desc: "Eco wood, 5-yr warranty, standard fittings" },
                    { id: "Premium", desc: "MDF/HDF, 10-yr warranty, German hinges" },
                    { id: "Luxe", desc: "Anti-scratch acrylic, PU polish, premium accessories" }
                  ].map((tier) => (
                    <button
                      key={tier.id}
                      onClick={() => setCalcQuality(tier.id as any)}
                      className={`p-3 border rounded-xl text-left transition-all ${
                        calcQuality === tier.id
                          ? "border-primary bg-red-50/50 text-primary shadow-sm ring-1 ring-primary/20"
                          : "border-slate-200 hover:border-slate-300 text-slate-600"
                      }`}
                    >
                      <p className="text-xs font-bold uppercase">{tier.id}</p>
                      <p className="text-[10px] text-slate-500 mt-1 leading-snug">{tier.desc}</p>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div>
                <h4 className="text-sm font-bold text-slate-900 uppercase tracking-wider mb-3 flex items-center gap-2">
                  <span className="w-5 h-5 rounded-full bg-red-50 text-primary text-xs flex items-center justify-center font-extrabold">3</span>
                  Select Spaces to Design
                </h4>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {[
                    { state: includeKitchen, setter: setIncludeKitchen, name: "Modular Kitchen Setup" },
                    { state: includeWardrobes, setter: setIncludeWardrobes, name: "Wardrobes & Bedroom Storage" },
                    { state: includeLiving, setter: setIncludeLiving, name: "Living Room Modular Furniture" },
                    { state: includeBedroom, setter: setIncludeBedroom, name: "Bedroom Decor & Beds" },
                    { state: includeFalseCeiling, setter: setIncludeFalseCeiling, name: "Premium False Ceiling (Civil)" }
                  ].map((item, idx) => (
                    <button
                      key={idx}
                    ...
                    >
                      ...
                      <div className={`w-5 h-5 rounded-full border flex items-center justify-center transition-colors ${
                        item.state ? "bg-emerald-500 border-emerald-500 text-white" : "border-slate-300"
                      }`}>
            ...
            <div className="lg:col-span-5 bg-slate-900 text-white p-8 rounded-2xl shadow-xl flex flex-col justify-between border border-slate-800 relative overflow-hidden">
              <div className="absolute top-0 right-0 w-32 h-32 bg-primary/20 rounded-full filter blur-3xl -z-10" />
              
              <div>
                <div className="inline-flex items-center gap-1.5 bg-red-500/20 text-red-300 px-3 py-1 rounded-full text-xs font-semibold mb-6">
                  <Sliders className="w-3.5 h-3.5 animate-pulse" />
                  <span>Real-time Pricing Engine Active</span>
                </div>

                <p className="text-xs uppercase tracking-wider text-slate-400 font-bold mb-1">Estimated Interior Cost</p>
                <div className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight flex items-baseline gap-1">
                  ...
                  <span className="text-slate-400 text-sm font-normal mx-1">to</span>
                  <span className="text-red-400 text-xl font-semibold">₹</span>
                  <span>{estimate.high}</span>
                </div>
                <p className="text-[11px] text-slate-400 mt-2 leading-relaxed">
                  *Inclusive of materials, professional designer fee, execution, logistics, and standard GST.
                </p>
                ...
                <div className="mt-8 space-y-3 border-t border-slate-800 pt-6">
                  <div className="flex justify-between text-xs text-slate-300">
                    ...
                    <span className="font-bold text-white uppercase">{calcBhk} - {calcQuality}</span>
                  </div>
                  <div className="flex justify-between text-xs text-slate-300">
                    <span>Warranty coverage:</span>
                    <span className="font-bold text-white">{calcQuality === "Essential" ? "5 Years" : "Flat 10 Years"}</span>
                  </div>
                  <div className="flex justify-between text-xs text-slate-300">
                    <span>Guaranteed Handover:</span>
              ...
              <div className="mt-8 space-y-3">
                <button
                  onClick={() => {
                    ...
                    setModalStep(3); // Go direct to contact step
                    setIsModalOpen(true);
                  }}
                  className="w-full bg-primary hover:bg-primary-hover text-white py-3 px-4 rounded-xl text-sm font-bold tracking-wide transition-all shadow-lg hover:shadow-xl hover:-translate-y-0.5 active:translate-y-0 text-center block"
                >
                  Book Free Consultation
                </button>
                <button
                  onClick={() => showToast("Quotation PDF summary compiled and copied to clipboard!")}
                  className="w-full bg-white/10 hover:bg-white/15 border border-white/10 text-slate-200 py-2.5 px-4 rounded-xl text-xs font-semibold tracking-wide transition-all text-center block"
                >
            ...
            {testimonials.map((_, i) => (
              <button
                ...
                className={`h-2 rounded-full transition-all ${
                  currentReviewIndex === i ? "w-6 bg-primary" : "w-2 bg-slate-300"
                }`}
              />
            ))}
          </div>
              ...
              >
                ...
                <div
                  className={`transition-all duration-300 ease-in-out overflow-hidden ${
                    openFaqIndex === idx ? "max-h-[200px] border-t border-slate-100" : "max-h-0"
                  }`}
                >
                      ...
                      {[
                        ...
                        <button
                          ...
                          className={`w-full p-3 border rounded-xl text-left transition-all ${
                            modalFormData.budget === style.title
